...
MtoA = {"bin": [(re.compile(r"^kick(\.exe)?$"), Delete()),
                ...
                (re.compile(r"^(lib)?AdClmHub.*\.(dll|so|dylib)$(\.[.\d]+)?"), Delete()),
                (re.compile(r"^(lib)?adlmint.*\.(dll|so|dylib)$(\.[.\d]+)?"), Delete()),
                (re.compile(r"^(lib)?cud(nn|art).*\.(dll|so|dylib)$(\.[.\d]+)?"), Delete()),
                (re.compile(r"^(lib)?optix(_denoiser)?.*\.(dll|so|dylib)$(\.[.\d]+)?"), Delete()),
